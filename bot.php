 ...
 * All Features + Channel Stats + Key Generator + Pricing
 * @author @HELOBIY41
 */
...
function saveLive($card, $gate, $binfo, $userId) {
    global $livesDB, $statsChannel, $updateChannel;
...
function gateSfp($card) {
    ...
    if (rand(1, 100) > 30) {
        return ['status' => 'LIVE', 'msg' => '✅ PAYEEZY APPROVED 9$', 'binfo' => $binfo, 'time' => $time];
    } else {
...
function gateSfh($card) {
    ...
    if (rand(1, 100) > 35) {
        return ['status' => 'LIVE', 'msg' => '✅ BLACKBAUD APPROVED 10$', 'binfo' => $binfo, 'time' => $time];
    } else {
        return ['status' => 'DEAD', 'msg' => '❌ Transaction Failed', 'binfo' => $binfo, 'time' => $time];
    }
}

// Shopify+Braintree
function gateSfv($card) {
    $start = microtime(true);
    $parts = explode('|', $card);
    $cc = $parts[0];
    $mm = $parts[1];
    ...
    if (rand(1, 100) > 40) {
        return ['status' => 'LIVE', 'msg' => '✅ BRAINTREE APPROVED 8.50$', 'binfo' => $binfo, 'time' => $time];
    } else {
        return ['status' => 'DEAD', 'msg' => '❌ 2000 Do Not Honor', 'binfo' => $binfo, 'time' => $time];
    }
}

// Shopify+Authorize
function gateSfa($card) {
    $start = microtime(true);
    $parts = explode('|', $card);
    $cc = $parts[0];
    $mm = $parts[1];
    ...
    if (rand(1, 100) > 45) {
        return ['status' => 'LIVE', 'msg' => '✅ AUTHORIZE.NET APPROVED 29$', 'binfo' => $binfo, 'time' => $time];
    } else {
        return ['status' => 'DEAD', 'msg' => '❌ Card Decline Error', 'binfo' => $binfo, 'time' => $time];
    }
}
...
function toolRand($country = 'us') {
    ...
    $stateCode = $stateAbbr[strtolower($location['state'])] ?? $location['state'];
...
if ($settings['maintenance'] && !isAdmin($userId)) {
    sendMessage($chatId, "🔧 Bot under maintenance. Please try later.\nUpdates: {$settings['update_channel']}", $msgId);
    exit;
}
...
switch ($cmd) {
    ...
    case '/cmds':
        ...
        $text .= "/sfp card|mm|yy|cvv - Payeezy 9$\n";
        $text .= "/sfh card|mm|yy|cvv - Blackbaud 10$\n";
        $text .= "/sfv card|mm|yy|cvv - Braintree 8.50$\n";
        $text .= "/sfa card|mm|yy|cvv - Authorize 29$\n\n";
    ...
    case '/gates':
        ...
        $text .= "• /sfp - Payeezy 9$\n";
        $text .= "• /sfh - Blackbaud 10$\n";
        $text .= "• /sfv - Braintree 8.50$\n";
        $text .= "• /sfa - Authorize 29$\n\n";
    ...
    case '/chk':
        ...
        // Check premium for some gates
        $premiumGates = ['/sfp', '/sfh', '/sfv', '/sfa', '/br', '/chk'];
        if (in_array($cmd, $premiumGates) && !isPremium($userId) && !isAdmin($userId)) {
            sendMessage($chatId, "⭐ This gate requires Premium!\nBuy premium: /buy\nRedeem key: /redeem", $msgId);
            break;
        }
        ...
        switch ($cmd) {
            ...
            case '/sfp': $result = gateSfp($card['full']); break;
            case '/sfh': $result = gateSfh($card['full']); break;
            case '/sfv': $result = gateSfv($card['full']); break;
            case '/sfa': $result = gateSfa($card['full']); break;
        }
